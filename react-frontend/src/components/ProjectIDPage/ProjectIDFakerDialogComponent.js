...
import projectIDFakerFactory from "./projectIDFakerFactory";
...
const ProjectIDFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = projectIDFakerFactory(props.user,fakerCount,UsernameIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("projectID").create(elem));
...
export default connect(mapState, mapDispatch)(ProjectIDFakerDialogComponent);
