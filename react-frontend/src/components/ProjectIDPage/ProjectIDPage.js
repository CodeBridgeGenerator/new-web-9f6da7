...
import ProjectIDDatatable from "./ProjectIDDataTable";
...
import ProjectIDFakerDialogComponent from "./ProjectIDFakerDialogComponent";
import ProjectIDSeederDialogComponent from "./ProjectIDSeederDialogComponent";
...
const ProjectIDPage = (props) => {
    ...
    useEffect(() => {
        ...
        client
            .service("projectID")
            .find({ query: { $limit: 10000 , Username : urlParams.singleProjectIDId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
    ...
    const service = _.find(config.services, { serviceName : "projectID"});
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("projectID").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "ProjectID", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
        ...
        const promises = data.map((e) => client.service("projectID").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
        ...
        <div className="mt-5">
            <div className="grid">
                <div className="col-6 flex justify-content-start">
                    <h3 className="mb-0 ml-2">ProjectID </h3>
                </div>
                <div className="col-6 flex justify-content-end">
                    <>
                    <Button label="add" loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="projectID-add-button"/>
            ...
            <div className="grid align-items-center">
                <div className="col-12" role="projectID-datatable">
                    <ProjectIDDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>
            ...
            <ProjectIDEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <ProjectIDCreateDialogComponent entity={newRecord} show={showCreateDialog} onHide={() => setShowCreateDialog(false)} onCreateResult={onCreateResult} />
            <ProjectIDFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <ProjectIDSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(ProjectIDPage);
