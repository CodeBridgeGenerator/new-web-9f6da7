...
const ProjectIDDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const dropdownTemplate1 = (rowData, { rowIndex }) => <p >{rowData.Username?.Username}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "projectID"});
...
<Column field="Username" header="Username" body={dropdownTemplate1} filter={selectedFilterFields.includes("Username")} hidden={selectedHideFields?.includes("Username")}  style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload ProjectID Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService />
      </Dialog>

      <Dialog header="Search ProjectID" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default ProjectIDDataTable;