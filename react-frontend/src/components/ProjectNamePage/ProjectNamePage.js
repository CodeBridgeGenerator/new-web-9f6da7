...
import ProjectNameDatatable from "./ProjectNameDataTable";
import ProjectNameEditDialogComponent from "./ProjectNameEditDialogComponent";
import ProjectNameCreateDialogComponent from "./ProjectNameCreateDialogComponent";
import ProjectNameFakerDialogComponent from "./ProjectNameFakerDialogComponent";
import ProjectNameSeederDialogComponent from "./ProjectNameSeederDialogComponent";
...
const ProjectNamePage = (props) => {
    ...
    useEffect(() => {
        ...
        client
            .service("projectName")
            .find({ query: { $limit: 10000 , ProjectName : urlParams.singleProjectIDId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
    ...
    const service = _.find(config.services, { serviceName : "projectName"});
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("projectName").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "ProjectName", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
        ...
        const promises = data.map((e) => client.service("projectName").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
        ...
        <div className="mt-5">
            <div className="grid">
                <div className="col-6 flex justify-content-start">
                    <h3 className="mb-0 ml-2">ProjectName </h3>
                </div>
                <div className="col-6 flex justify-content-end">
                    <>
                    <Button label="add" loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="projectName-add-button"/>
            ...
            <div className="grid align-items-center">
                <div className="col-12" role="projectName-datatable">
                    <ProjectNameDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>
            ...
            <ProjectNameEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <ProjectNameCreateDialogComponent entity={newRecord} show={showCreateDialog} onHide={() => setShowCreateDialog(false)} onCreateResult={onCreateResult} />
            <ProjectNameFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <ProjectNameSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(ProjectNamePage);
