...
import projectNameFakerFactory from "./projectNameFakerFactory";
...
const ProjectNameFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = projectNameFakerFactory(props.user,fakerCount,ProjectNameIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("projectName").create(elem));
...
export default connect(mapState, mapDispatch)(ProjectNameFakerDialogComponent);
