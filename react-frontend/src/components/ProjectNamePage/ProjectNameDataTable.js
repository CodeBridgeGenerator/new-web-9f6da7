...
const ProjectNameDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const dropdownTemplate0 = (rowData, { rowIndex }) => <p >{rowData.ProjectName?.name}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "projectName"});
...
<Column field="ProjectName" header="ProjectName" body={dropdownTemplate0} filter={selectedFilterFields.includes("ProjectName")} hidden={selectedHideFields?.includes("ProjectName")}  style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload ProjectName Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService />
      </Dialog>

      <Dialog header="Search ProjectName" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default ProjectNameDataTable;